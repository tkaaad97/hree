...
lib: src shader package.yaml
	docker-compose run --rm app stack build --flag hree:-embed_shaders --test --no-run-tests
...
test: libglxoverride.so
	docker-compose run --rm -e LD_PRELOAD=./libglxoverride.so app stack test --flag hree:-embed_shaders hree:hree-test
