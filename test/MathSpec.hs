        ...
        it "should decompose matrix to transform" . property $
            \((tx, ty, tz), (q0, q1, q2, q3), (sx, sy, sz)) -> do
                let translate = V3 tx ty tz
                    quaternion = normalize (Quaternion (correctNotZero q0) (V3 (correctNotZero q1) q2 q3))
                    scale_ = V3 (abs $ correctNotZero sx) (abs $ correctNotZero sy) (abs $ correctNotZero sz)
